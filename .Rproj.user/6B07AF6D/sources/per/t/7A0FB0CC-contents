...
#'   \item{mononucleosome: total number of fragments with length between 147
#'   bp and 294 bp}
...
#' @examples
#' fpath <- system.file("extdata", "fragments.tsv.gz", package="SignacSlim")
#' name_path <- system.file("extdata", "cell_name.txt", package="SignacSlim")
#' tmpf <- tempfile(fileext = ".gz")
#' name <- readLines(name_path)
#' FilterCells(
#'   fragments = fpath,
#'   cells = name,
#'   outfile = tmpf
...
SplitFragments <- function(
    ...
    for (i in seq_along(along.with = frags)) {
        ...
        if (isRemote(x = fragpath)) {
            ...
            idents.use <- as.character(
                x = unname(obj = groups[names(x = cells.use)])
                )
