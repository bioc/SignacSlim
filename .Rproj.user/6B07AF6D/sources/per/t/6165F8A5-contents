...
#' @examples
#' if(FLASE){
#'     print("see https://satijalab.org/signac/articles/pbmc_vignette.html")
#' }
TSSEnrichment <- function(
    ...
    if (!is.null(x = n)) {
        ...
        tss.positions <- tss.positions[seq_len(n), ]
    }
    ...
    # if the flanking mean is 0 for any cells,
    # the enrichment score will be zero.
    ...
    print("Please ignore the above Warnings.")
    return(object)
}
...
TSSFast <- function(
    ...
    for (i in seq_along(along.with = frags)) {
        ...
        if (is.null(x = cellmap)) {
            ...
            cellmap <- cellmap[intersect(names(x = cellmap),
                                         colnames(x = object))]
        }
